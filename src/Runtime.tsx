...
import { Workflow } from "./utils/workflow/types"
import execWorkflow from "./utils/workflow"



const Runtime: FC<ComponentConfig & {
    customComponents: Record<string, ComponentConfig>,
    slotChildren?: GridLayout[],
    slotGlobalProps?: Record<string, any>,
    ...
    let globalActions = useMemo(() => {
        ...
        if (initConfig.workflows) {
            Object.entries(initConfig.workflows).forEach(([key, workflow]) => {
                newActions[key] = () => {
                    execWorkflow({ workflow, globalState: { state: globalData, setState: () => Promise.resolve(), navigate: newActions.navigate }, localState: {}, setLocalState: () => Promise.resolve(), props: {} })
                }
            })
        }
        return newActions
    }, [initConfig.actions, initConfig.workflows])
        ...
        >
            {gridLayout.length ? gridLayout?.map((item) => {
                ...
                return <SubRuntime
                    ...
                    actions={globalActions || {}}
                    parentState={componentStateAsProps || {}}
                    slotChildren={item.slotChildren}
